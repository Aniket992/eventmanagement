...
const Registration = () => {
  ...
  const totalFee = formData.selectedEvents.reduce(
    (total, eventId) => {
      const event = events.find((evt) => evt._id === eventId);
      return event
        ? total + parseInt(event.registrationCharge.amount || 0)
        : total;
    },
    ...
    >
      <div className="bg-black bg-opacity-95 min-h-screen h-full">
        ...
        <div className="flex flex-col items-center justify-center mt-5 w-full ">
          <h1 className="font-avenger tracking-widest text-4xl text-center text-white mb-3">
            Registration Form
          </h1>
          <form
          ...
          >
            <div className="rounded-xl text-xl justify-between text-yellow-400 font-serif flex flex-col tracking-widest  w-full md:w-1/2 ">
              <h1 className="text-center font-normal text-3xl text-fuchsia-500">
                Student Details
              </h1>
              <div className="mb-4 mt-1">
                <label className="block mb-1 tracking-wides font-avenger font-normal">
                  Student Name:
                </label>
                <input
                  type="text"
              ...
              <div className="flex justify-between gap-2 mb-4">
                ...
                <div className="flex flex-col justify-between w-1/2">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    required
                    className="w-full p-1.5  text-center border rounded bg-transparent"
                  >
                    <option className="bg-black text-center " value="">
                      Select Class
                    </option>
                    <option className="bg-black text-center" value="6th">
                      6th
                    </option>
                    <option className="bg-black text-center" value="7th">
                      7th
                    </option>
                    <option className="bg-black text-center" value="8th">
                      8th
                    </option>
                    <option className="bg-black text-center" value="9th">
                      9th
                    </option>
                    <option className="bg-black text-center" value="10th">
                      10th
                    </option>
                    <option className="bg-black text-center" value="11th">
                      11th
                    </option>
                    <option className="bg-black text-center" value="12th">
                      12th
                    </option>
                  </select>
                </div>
              ...
              <div className="">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  required
                  className="w-full min-h-11 max-h-32 px-3 py-2 border rounded bg-transparent"
                ></textarea>
              </div>
            </div>
...
<button type="button" onClick={handleSendOtp} className="mt-2 px-3 py-1 bg-purple-500 text-white rounded" disabled={otpSent || otpLoading}>
                ...
                {emailVerified ? (
                  ...
                  <div className="mb-4 flex  font-serif justify-between items-end ">
                    <button
                      ...
                      className={`p-2 h-fit w-52 bg-blue-500 text-white rounded  ${
                        otpLoading ? "opacity-50 cursor-not-allowed" : ""
                      }`}
                    >
                      {otpCooldown
                        ? `resend otp 00: ${countdown}`
                        : otpLoading
                        ? "Sending..."
                        : "Send OTP"}
                    </button>
                      ...
                      <>
                        <div>
                          ...
                          <input
                            ...
                            className=" w-32 px-3 py-2 border rounded bg-transparent"
                          />
                        </div>
                        ...
                        <button
                          ...
                          className={`p-2 h-fit bg-green-500 text-white rounded mt-2 ${
              ...
              <div className="mb-6 flex flex-col gap-3 border h-80 overflow-y-scroll ">
                {events &&
                  events.map((evt) => (
                    <div
                      key={evt._id}
                      className="flex items-center rounded  mb-2 justify-between px-3"
                    >
                      <input
                        type="checkbox"
                        name="selectedEvents"
                        ...
                        className="w-10 h-7"
                      />
                      <label className="text-white tracking-widest text-xl">
                        {evt.eventName && evt.eventName}
                      </label>
                      <label className="text-orange-400 text-2xl">
                        {evt.registrationCharge.currency &&
                          evt.registrationCharge.currency}{" "}
                        {evt.registrationCharge.amount &&
                          evt.registrationCharge.amount}
                      </label>
                    </div>
                  ))}
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white py-2 px-4 text-xl tracking-widest rounded hover:bg-blue-700"
              >
                Pay
              </button>
            </div>
